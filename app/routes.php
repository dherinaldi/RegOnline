...
$app->get('/api/carabayar/', function($request, $response, $args) {
	$getRecord = $request->getParsedBody();
	$getHeader = $request->getHeaders();
	$action = "getCaraBayar?start=0&limit=300";
	$method = "GET";
	$token = getApiToken('getToken','POST');
	$params = '';
	$result = sendRequest($action,$method,$params,$token);
...
$app->get('/api/carirujukankartutes/', function($request, $response, $args) {
	$getHeader = $request->getHeaders();
	$getRecord = $request->getParsedBody();
	$nik = "6307061712980002";
	$nomor = "0001800300993";
	$action = "getListRujukanKartu";
	$method = "GET";
	$token = getApiToken('getToken','POST');
	$data = array(
		"noKartu" => $nomor
	...
	$params = trim($params, '&');
	$result = sendRequest($action."?faskes=1&noKartu=".$nomor,$method,$params,$token);
	var_dump($result);
	$resultJsn1 = json_decode($result);
	
		
});
$app->get('/api/carikontrolkartutes/', function($request, $response, $args) {
	$getHeader = $request->getHeaders();
	$getRecord = $request->getParsedBody();
	//$nik = "6307061712980002";
	$nomor = "0001097097805";
	$action = "getListSuratKontrol";
	$method = "GET";
	$token = getApiToken('getToken','POST');
	$data = array(
		"noKartu" => $nomor
	...
	$params = trim($params, '&');
	$result = sendRequest($action."?tanggal=2023-10-30&nokartu=".$nomor,$method,$params,$token);
	var_dump($result);
	$resultJsn1 = json_decode($result);
	
		
});
$app->post('/api/carirujukankartu/', function($request, $response, $args) {
	$getHeader = $request->getHeaders();
	$getRecord = $request->getParsedBody();
	$nik = isset($getRecord['nik']) ? $getRecord['nik'] : '';
	$nomor = isset($getRecord['noKartu']) ? $getRecord['noKartu'] : '';
	$faskes = isset($getRecord['faskes']) ? $getRecord['faskes'] : 2;
	$action = "getListRujukanKartu";
	$method = "GET";
	$token = getApiToken('getToken','POST');
	$data = array(
	...
	$params = trim($params, '&');
	$result = sendRequest($action."?faskes=".$faskes."&noKartu=".$nomor,$method,$params,$token);
	
	$resultJsn1 = json_decode($result);
	if(!$resultJsn1->success){
		return $response->withJson(["success" => false,"message" => $resultJsn1->message],201);
	} else {
		return $response->withJson(["success" => true,"data" => $resultJsn1->data,"message" => "Data Rujukan Di Temukan"],202);
	}
		
});
$app->post('/api/carikontrolkartu/', function($request, $response, $args) {
	$getHeader = $request->getHeaders();
	$getRecord = $request->getParsedBody();
	//$nik = isset($getRecord['nik']) ? $getRecord['nik'] : '';
	$nomor = isset($getRecord['nokartu']) ? $getRecord['nokartu'] : '';
	...
	$params = trim($params, '&');
	$result = sendRequest($action."?tanggal=".$tanggal."&nokartu=".$nomor,$method,$params,$token);
	
	$resultJsn1 = json_decode($result);
	if(!$resultJsn1->success){
		return $response->withJson(["success" => false,"message" => $resultJsn1->message],201);
	} else {
		return $response->withJson(["success" => true,"data" => $resultJsn1->data,"message" => "Data Kontrol Di Temukan"],202);
	}
		
});
$app->post('/api/cariposantrianpenjamin/', function($request, $response, $args) {
	$getHeader = $request->getHeaders();
	$getRecord = $request->getParsedBody();
	$penjamin = isset($getRecord['penjamin']) ? $getRecord['penjamin'] : '';
	$action = "getPosPenjamin";
	$method = "GET";
	$token = getApiToken('getToken','POST');
	$data = array(
		"PENJAMIN" => $penjamin
	...
	$params = trim($params, '&');
	$result = sendRequest($action."?PENJAMIN=".$penjamin,$method,$params,$token);
	$resultJsn1 = json_decode($result);
	if(!$resultJsn1->success){
		return $response->withJson(["success" => false,"message" => $resultJsn1->message],201);
	} else {
		return $response->withJson(["success" => true,"data" => $resultJsn1->response,"message" => "Data Rujukan Di Temukan"],202);
	}
...
$app->get('/libur[/]', function($request, $response) {
	return $this->view->render($response, 'home.php', 
		array(
			'title' => 'RSU Syifa Medika',
			'labelInfo' => 'Daftar',
...
$app->get('/step2/{jenis}/{norm}/{nama}/{tmptLahir}/{tglLahir}/{nik}', function($request, $response, $args) {
	$getRecord = $request->getParsedBody();
