'use Home';
var CtrlStepVerifikasiRujukan = function () {
    return {
        init: function () {
			...
			CtrlStepVerifikasiRujukan.onLoadCaraBayar(cb);
			CtrlStepVerifikasiRujukan.onLoadRuangan(poli);
			CtrlStepVerifikasiRujukan.loadKomponenNewForm();
        },
		...
		onLoadRuangan: function (rec) {
			...
			$.ajax({
				type: "GET",
				url: globalWsPath+"/tujuanrujukan/",
				cache: true,
				headers : {
					"KeyApi": '808'
				},
				...
				success: function(response){
					if(response.success){
						var jmlData = response.data.length;
						for(var i= 0; i<jmlData; i++){
							if(response.data[i].KDPOLI == rec){
								$("#poliTjuanTemp").html(response.data[i].NMPOLI);
							}
						}
					}
...
CtrlStepVerifikasiRujukan.init();